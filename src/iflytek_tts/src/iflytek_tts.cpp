...
int flag = 0;
static std::string appID = "5c4ac2f8";      //default appID //
static std::string speakerName = "xiaoyan"; //default speaker name
...
{
    int ret = MSP_SUCCESS;
    const char *login_params = "appid = 5c4ac2f8, work_dir = ."; //登录参数,appid与msc库绑定,请勿随意改动
    const char *filename = path;                                 //合成的语音文件名称
    const char *session_begin_params = "voice_name = xiaoyan, text_encoding = utf8, sample_rate = 16000, speed = 50, volume = 50, pitch = 50, rdn = 2";
    string greet_word = "木星语音系统启动成功！";                //default need composed greet word
	...
	*/
    ...
    /* 文本合成 */
    printf("开始合成 ...\n");
    ret = text_to_speech(greet_word.c_str(), filename, session_begin_params);
    if (MSP_SUCCESS != ret)
    {
        printf("text_to_speech failed, error code: %d.\n", ret);
    }
    ...
    ros::Subscriber sub = n.subscribe(tts_topic, 1, xfcallback);
    playVoice();
    ros::spin();
