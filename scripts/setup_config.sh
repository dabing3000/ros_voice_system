...
#       软件更快;
#    2020.3.8 将pocketsphinx更改为pocketsphinx_zh，避免重复。
#             不再将讯飞的libmsc.so拷贝到usr/lib下，方便用户更换自己的ID和动态库。
#
#############################################################################
...
sudo cp ${WORKSPACE_PATH}src/pocketsphinx_zh/model/*.DMP ${LM_PATH}
sudo cp ${WORKSPACE_PATH}src/pocketsphinx_zh/model/*.dic ${LM_PATH}
HMM_PATH=/usr/share/pocketsphinx/model/hmm/zh/
sudo mkdir -p ${HMM_PATH}
sudo cp -rf ${WORKSPACE_PATH}src/pocketsphinx_zh/model/zh_broadcastnews_ptm256_8000 ${HMM_PATH}
#install neceesary apt-get pkg and debian pkg
cd ${WORKSPACE_PATH}src/pocketsphinx_zh/debian/
